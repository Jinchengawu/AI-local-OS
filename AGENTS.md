...
**扩展文档（AI-local-OS）**：[docs/ai-local-os/README.md](docs/ai-local-os/README.md) — OpenClaw + Hermes 私有化集成模板与规格索引（与 monorepo 业务包无强耦合）。**Node 工作区**：根目录 `pnpm-workspace.yaml`，包见 `packages/openclaw`、`packages/hermes-agent` 与 [monorepo-packages.md](docs/ai-local-os/monorepo-packages.md)。
