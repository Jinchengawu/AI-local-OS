...
Bug 通常在调用栈深处表现出来（git init 在错误目录、文件在错误位置创建、数据库用错误路径打开）。你的本能是在错误出现的地方修复，但那是在治标。
...
使用本目录下的二分法脚本 `find-polluter.sh`：

```bash
./find-polluter.sh '.git' 'src/**/*.test.ts'
```

逐个运行测试，在找到第一个污染者时停止。详见脚本的用法。

## 真实示例：空 projectDir

**症状：** `.git` 在 `packages/core/`（源代码目录）中被创建
...
- 第 3 层：NODE_ENV 守卫拒绝在 tmpdir 之外运行 git init
...
digraph principle {
    ...
    "找到直接原因" -> "能再往上追踪一层？";
    "能再往上追踪一层？" -> "反向追踪" [label="是"];
    "能再往上追踪一层？" -> "绝不只修复症状" [label="否"];
    "反向追踪" -> "这是源头吗？";
    "这是源头吗？" -> "反向追踪" [label="否 - 继续往上"];
    "这是源头吗？" -> "在源头修复" [label="是"];
    "在源头修复" -> "在每一层添加验证";
    "在每一层添加验证" -> "Bug 不可能再发生";
}
