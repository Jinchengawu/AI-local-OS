...
description: Use when you need to install or update web3-fe-superpowers skills to .agents, .cursor/skills, and .cursor/skills directories, or when user asks to sync/update web3 frontend development skills
---
...
- **Skills**：安装到 `.agents/skills`、`.cursor/skills`、`.cursor/skills`
- **Agents**：安装到 `.agents/agents`、`.cursor/agents`、`.cursor/agents`
...
| 检查 .claude skills | `ls -la .cursor/skills/` |
| 检查 .claude agents | `ls -la .cursor/agents/` |
...
远程 skills 中包含 `.cursor/` 路径引用，安装时自动适配：

- **安装到 `.agents`** → 将 `.cursor/` 替换为 `.agents/`
- **安装到 `.claude`** → 保持 `.cursor/` 不变
- **安装到 `.cursor`** → 将 `.cursor/` 替换为 `.cursor/`
...
- **`.cursor/skills`**：Claude Code skills，保持原始路径
- **`.cursor/agents`**：Claude Code agents，保持原始路径
...
# 或 ./.cursor/skills/dev-ops/install-web3-skills.sh
```
...
4. 智能合并到 `.cursor/skills`（保持原路径）
5. 智能合并到 `.cursor/agents`（保持原路径，如果远程有）
...
路径适配会自动将文件中的 `.cursor/` 引用替换为目标目录路径。
...
- **路径安全**：仅替换 `.md` 和 `.sh` 文件中的 `.cursor/` 引用
