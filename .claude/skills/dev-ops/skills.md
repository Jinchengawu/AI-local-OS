...
description: Use when you need to install or update web3-fe-superpowers skills to .agents, .claude/skills, and .cursor/skills directories, or when user asks to sync/update web3 frontend development skills
---
...
- **Skills**：安装到 `.agents/skills`、`.claude/skills`、`.cursor/skills`
- **Agents**：安装到 `.agents/agents`、`.claude/agents`、`.cursor/agents`
...
| 检查 .claude skills | `ls -la .claude/skills/` |
| 检查 .claude agents | `ls -la .claude/agents/` |
...
远程 skills 中包含 `.claude/` 路径引用，安装时自动适配：

- **安装到 `.agents`** → 将 `.claude/` 替换为 `.agents/`
- **安装到 `.claude`** → 保持 `.claude/` 不变
- **安装到 `.cursor`** → 将 `.claude/` 替换为 `.cursor/`
...
- **`.claude/skills`**：Claude Code skills，保持原始路径
- **`.claude/agents`**：Claude Code agents，保持原始路径
...
# 或 ./.claude/skills/dev-ops/install-web3-skills.sh
```
...
4. 智能合并到 `.claude/skills`（保持原路径）
5. 智能合并到 `.claude/agents`（保持原路径，如果远程有）
...
路径适配会自动将文件中的 `.claude/` 引用替换为目标目录路径。
...
- **路径安全**：仅替换 `.md` 和 `.sh` 文件中的 `.claude/` 引用
