...
AGENTS_DIR=".agents"
CLAUDE_SKILLS_DIR=".agents/skills"
CLAUDE_AGENTS_DIR=".agents/agents"
...
install_to_skills_dir() {
    ...
    for skill in "${REMOTE_SKILLS[@]}"; do
        ...
        # 路径自动适配：替换文件中的 .agents/ 引用为目标路径前缀
        # 注意：仅当远程 skills 包含 .agents/ 引用时才会生效
        if [ "$path_prefix" != ".claude" ]; then
            # 查找包含 .agents/ 的文件
            local files_with_claude_ref=$(find "$target_dir/$skill" -type f \( -name "*.md" -o -name "*.sh" \) -exec grep -l "\.agents/" {} \; 2>/dev/null || true)

            if [ -n "$files_with_claude_ref" ]; then
                echo "  🔧 Adapting paths: .agents/ → $path_prefix/"
                echo "$files_with_claude_ref" | while read -r file; do
                    # 跨平台的 sed 替换（兼容 macOS 和 Linux）
                    if sed --version 2>&1 | grep -q GNU; then
                        # GNU sed (Linux)
                        sed -i "s|\.agents/|$path_prefix/|g" "$file"
                    else
                        # BSD sed (macOS)
                        sed -i '' "s|\.agents/|$path_prefix/|g" "$file"
                    fi
...
install_to_agents_dir() {
    ...
    for agent in "${REMOTE_AGENTS[@]}"; do
        ...
        # 路径自动适配（agents 文件也可能引用 .agents/）
        if [ "$path_prefix" != ".claude" ]; then
            local files_with_claude_ref=$(find "$target_dir/$agent" -type f \( -name "*.md" -o -name "*.sh" \) -exec grep -l "\.agents/" {} \; 2>/dev/null || true)
...
# ========================================
# 2. 安装到 .agents/（skills + agents）
# ========================================

# 2.1 安装 skills
install_to_skills_dir "$CLAUDE_SKILLS_DIR" ".agents/skills" ".claude"

# 2.2 安装 agents
if [ -d "${CONTENT_ROOT}/agents" ]; then
    install_to_agents_dir "$CLAUDE_AGENTS_DIR" ".agents/agents" ".claude"
fi
...
echo "  .agents/skills/:  $(ls -1 "$CLAUDE_SKILLS_DIR/" 2>/dev/null | wc -l | xargs) skills"
echo "  .agents/agents/:  $(ls -1 "$CLAUDE_AGENTS_DIR/" 2>/dev/null | wc -l | xargs) agents"
